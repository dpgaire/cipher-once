-- Function to update a secret's view count and burn it if max views are reached.
-- This version no longer deletes the file, to prevent a race condition.
-- This version re-introduces the COALESCE fix from script 017.
...
BEGIN
  ...
  IF FOUND AND NOT v_is_burned THEN
    ...
    -- Update the secret's view count and burned status
    UPDATE secrets
    SET
    ...
    -- If a user is associated with the secret, update their view stats
    IF v_user_id IS NOT NULL THEN
      UPDATE profiles
      SET total_secrets_viewed = COALESCE(total_secrets_viewed, 0) + 1
      WHERE id = v_user_id;

      -- If the secret was burned, update their burned stats
      IF v_should_burn AND v_user_id IS NOT NULL THEN
        UPDATE profiles
