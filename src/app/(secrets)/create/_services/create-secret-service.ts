import {
  ...
  base64ToBuffer,
} from "@/app/(secrets)/s/[shortId]/_services/encryption";
import {
  calculateExpirationDate,
  validatePassphrase,
  SecretMetadata,
} from "@/lib/utils";
import { convertHeicToJpeg } from "../_utils/heic-converter";
...
async function handleFileUpload(
  ...
  try {
    ...
    if (fileToProcess.size > MAX_CLIENT_FILE_SIZE) {
      throw new Error(
        "File too large. Please choose a smaller file (max 20MB)."
      );
    }
